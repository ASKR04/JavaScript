...
Unavailable browser storage is also explicit at startup. The toolbar identifies the workspace as temporary, offers an immediate storage probe, and keeps local backup download available. Once work changes, a shared exit policy warns while persistence is pending, failed, conflicted, or paused for unreadable-data recovery, preventing an unacknowledged close from discarding the only in-memory copy.
